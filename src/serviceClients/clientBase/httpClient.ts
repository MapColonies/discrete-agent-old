...
export abstract class HttpClient {
  ...
  private wrapError(url: string, err: AxiosError, body?: unknown): HttpError {
    const message = get(err, 'response.data.message', undefined) as string | undefined;
    switch (err.response?.status) {
      ...
      case HttpStatus.CONFLICT:
        this.logger.log('debug', `conflict on request for service ${this.targetService}, target url: ${url}, error: ${err.message}`);
