...
public constructor(private readonly shpParser: ShpParser){}
    public async trigger(directory: string){
        //TODO: get history
        ...
        if(await this.fileExists(filesShp) && await this.fileExists(filesDbf) &&
        await this.fileExists(productShp) && await this.fileExists(productDbf) &&
        await this.fileExists(metadataShp) && await this.fileExists(metadataDbf)) {
            //read file list
